...
function end_of_quiz() {
    ...
    let currHours = curr_date.getHours();
    let currMinutes = curr_date.getMinutes();
    let ampm = currHours >= 12 ? 'PM' : 'AM';
    currHours = currHours % 12;
    currHours = currHours ? currHours : 12;
    let formattedMinutes = currMinutes < 10 ? '0' + currMinutes : currMinutes;
    let formattedTime = `${currHours}:${formattedMinutes} ${ampm}`;
    let dateTimeGenerated = curr_month[curr_date.getMonth()] + ` ` + curr_date.getDate() + `, ` + curr_date.getFullYear()  + `, ` + curr_day[curr_date.getDay()] + `, ` + formattedTime;
    ...
    let durationMinutes = Math.floor((timerDuration % 3600) / 60); // Calculate minutes within the current hour
    let seconds = Math.floor(timerDuration % 60);
    let durationHours = Math.floor(timerDuration / 3600);
    ...
    if (durationHours !== 0) {
        txtHours = `${durationHours} hour(s) and`;
    }
    if (durationMinutes !== 0) {
        txtMinutes = `${durationMinutes} minute(s) and`;
    }
...
function showResult(showResultTopicID) {
    ...
    // Convert the timer counter into hours, minutes, and seconds
    let timerDuration = Math.floor(timerCounterForGlobal);
    let durationMinutes = Math.floor((timerDuration % 3600) / 60);
    let seconds = Math.floor(timerDuration % 60);
    let durationHours = Math.floor(timerDuration / 3600);
    ...
    for (var i = 0; i < quiz_scoreboard_topic.length; i++) {
        $("#score_list_quiz").append(`
            <tr>
                ...
                <td style="width:540px;text-align:left"><span class="analytical_info_rate">${quiz_scoreboard_topic[i]}</span></td>
                ...
                <td style="width:100px;"><span class="numerical_info_rate"><button class="review_answer ripple-btn" onclick="displayQuizPrevQuestionsAndChoices(${quiz_scoreboard_topicID[i]}, ${[i]})">Review</button></td>
                <td style="width:110px;"><span class="numerical_info_rate"><button class="retake_quiz ripple-btn" onclick="selectedQuiz(${quiz_scoreboard_topicID[i]})">Retake</button></td>
            </tr>
        `);
    }
    ...
    for (var i = 0; i < exam_scoreboard_topic.length; i++) {
        $("#score_list_exam").append(`
            <tr>
                ...
                <td style="width:100px;"><span class="numerical_info_rate"><button class="review_answer ripple-btn" onclick="reviewed_ans_for_exam(${[i]})">Review</button></td>
            </tr>
        `);
    }
